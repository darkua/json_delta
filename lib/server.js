...
dispatcher.onPost("/diff",function(req,res) {
  ...
  try{
    if(req.params.start === undefined && req.params.end === undefined){
      ...
      //diffs the data
      var out = _.mapObject(data, function(val, key) {
        //validate enouph data
        if(val.length < 2){
          return "No data in selected interval to diff changes"
        }
        var changes=[]
        for (var i = 0; i < val.length-1 ; i++) {
          var oldObj = val[i];
          var newObj = val[i+1];
          var diff = jsonDiff.diff(oldObj,newObj,["_id","_timestamp"]);
          changes.push((!diff)?"No changes":diff)
        }
        return changes;
      })
      res.end(JSON.stringify(out))
